...
| PostgreSQL 持久化 | **Implemented (limited)** | 持久化权威 revision、Session 聚合、SessionWorker、Artifact、File/FileVersion、ToolCall/Approval/SideEffect、失败/拒绝安全审计与幂等记录；34 张应用表 FORCE RLS，API/Worker 分离受限角色，transaction-local context 与跨 tenant 负向测试已实现；未配置数据库的开发模式使用内存 | 对象存储、数据库高可用、容量、在线迁移与回滚演练；生产 PITR/远端备份仍需部署证据 |
