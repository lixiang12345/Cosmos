...
| Space 管理 | **Implemented (limited)** | 生产 Web 使用服务端 Space authority；支持 Organization 内 list/create、metadata/defaults CAS 更新、Default Space、默认 published Expert/ready Environment、真实 membership scope 切换和删除迁移影响预览；Default 不可改名，未授权写入被拒绝 | 实际资源迁移 job、恢复/回滚、archive/delete、membership 邀请与角色管理 |
