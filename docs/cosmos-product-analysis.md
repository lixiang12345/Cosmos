# Augment Cosmos 产品研究与 Cosmos 完整原型基线

> 研究日期：2026-07-12  
> 资料范围：Augment 官方 Cosmos 文档、官方 Blog、Changelog、Workflows 目录  
> 目的：理解 Cosmos 的真实产品模型，并据此建立 Cosmos 后续原型与工程实现基线
...
Cosmos 的原型也必须围绕这些对象和关系构建。继续添加互不相干的 Dashboard、卡片或静态设置页，无法形成可用于后端设计的产品基线。
...
Cosmos 在工程实现中必须把 Session 和 Run Attempt 分开：
...
最新官方文档持续建议使用 Cosmos Advisor 配置 Integrations、Environments、Experts 和 Automations。传统表单仍然存在，但 Advisor 是首选入口。Cosmos 应同时保留：
...
官方没有公开具体数据库、消息队列、编排器或服务端语言，Cosmos 不应在产品原型阶段假设必须使用某个具体基础设施。
