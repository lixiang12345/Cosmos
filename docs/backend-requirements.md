...
| Session API | tenant-scoped list/create/get/rename/archive/restore/draft-start/send/pause/resume/cancel/retry、ShareGrant list/create/revoke 与 Artifact list/create/update/remove；Session/Artifact list 使用 tenant/filter-bound keyset cursor；Message/Event cursor 分页；可恢复 SSE；单资源和所有强版本写返回 `ETag`；共享 Zod 请求/响应/错误验证；Private 资源按 creator/grant conceal | Pin 尚未拆为 actor 偏好资源；无 File/Tool/Approval API |
