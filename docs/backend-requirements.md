...
| 权威配置与持久化 | PostgreSQL migration 已建立 Expert/Environment identity、immutable revision、Repository binding、User/Group ShareGrant、ServiceAccount exact binding、复合 tenant FK 与 24 张租户表的 FORCE RLS；Session 写事务原子维护 Message/Turn/Attempt/Command/Outbox/Event/Audit；Worker 写 Attempt、Agent Message 与有序事件 | 无 Expert/Environment CRUD/publish API、ExecutionSnapshot 或完整失败/拒绝 Audit；大表 migration 的生产规模锁等待仍需演练 |
...
| 测试 | API/repository/config/JWT 单元测试；配置 `TEST_DATABASE_URL` 时运行 PostgreSQL 并发幂等、权威配置、Catalog/Session 分页与可见性、metadata/执行控制/ShareGrant/ServiceAccount、受限角色/FORCE RLS/上下文防泄漏、跨 tenant/Private 隔离、Session FIFO、Worker 就绪边界和 `001 -> 047` 新库/升级测试；Session activity indexes 使用独立 concurrent migrations | 数据库测试会在无环境变量时 skip；无生产规模在线 schema、备份恢复或负载测试 |
