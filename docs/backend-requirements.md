...
| Session API | tenant-scoped list/create/get/rename/archive/restore/draft-start/send；Session list 支持 tenant/filter-bound keyset cursor、status/archive/search filter；Message/Event cursor 分页；可恢复 SSE；单资源和所有强版本写返回 `ETag`；共享 Zod 请求/响应/错误验证；Private 资源按 creator conceal | 无 pause/resume/cancel/retry/share 命令；Pin 尚未拆为 actor 偏好资源 |
...
| 写入幂等 | Organization + authenticated actor + method + canonical path + key 作用域；create/start/send/archive/restore 同 key/同 body 重放，不同 body 返回 409；PostgreSQL 使用事务级 advisory lock 处理并发；rename 使用强 `If-Match` CAS | 未运行过期记录清理作业；尚未统一所有写 endpoint 的幂等中间件 |
| 测试 | API/repository/config/JWT 单元测试；配置 `TEST_DATABASE_URL` 时运行 PostgreSQL 并发幂等、权威配置、Catalog/Session 分页与可见性、metadata CAS/账本、跨 tenant/Private 隔离、Session FIFO、Worker 就绪边界和 `001 -> 040` 新库/升级测试；Session activity indexes 使用独立 concurrent migrations | 数据库测试会在无环境变量时 skip；无 RLS、完整在线 schema 演练、备份恢复或负载测试 |
...
- ServiceAccount token 必须绑定 Organization、Space、Automation 和允许动作。
