...
| 身份发现与授权 | `GET /api/v1/me` 返回 authenticated actor 及其真实 Organization/Space membership；Session repository 重检 membership；Private Session 支持动态 User/Group ShareGrant；ServiceAccount 的 create/send/archive 使用 audience + exact Expert/Session binding，并在写事务内重检 | 无合规访问、RLS、实时撤权通知或公开 ServiceAccount binding 管理 API |
| Session API | tenant-scoped list/create/get/rename/archive/restore/draft-start/send/pause/resume/cancel/retry 与 ShareGrant list/create/revoke；Session list 支持 tenant/filter-bound keyset cursor、status/archive/search filter；Message/Event cursor 分页；可恢复 SSE；单资源和所有强版本写返回 `ETag`；共享 Zod 请求/响应/错误验证；Private 资源按 creator/grant conceal | Pin 尚未拆为 actor 偏好资源；无 File/Artifact/Tool/Approval API |
...
| 权威配置与持久化 | PostgreSQL migration 已建立 Expert/Environment identity、immutable revision、Repository binding、User/Group ShareGrant、ServiceAccount exact binding 和复合 tenant FK；Session 写事务原子维护 Message/Turn/Attempt/Command/Outbox/Event/Audit；Worker 写 Attempt、Agent Message 与有序事件 | 无 Expert/Environment CRUD/publish API、ExecutionSnapshot、完整失败/拒绝 Audit 或 RLS；大表 migration 仍需分阶段上线方案 |
...
| 测试 | API/repository/config/JWT 单元测试；配置 `TEST_DATABASE_URL` 时运行 PostgreSQL 并发幂等、权威配置、Catalog/Session 分页与可见性、metadata/执行控制/ShareGrant/ServiceAccount 策略、跨 tenant/Private 隔离、Session FIFO、Worker 就绪边界和 `001 -> 045` 新库/升级测试；Session activity indexes 使用独立 concurrent migrations | 数据库测试会在无环境变量时 skip；无 RLS、完整在线 schema 演练、备份恢复或负载测试 |
