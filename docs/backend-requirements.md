...
| Space API | Organization-scoped list/create 与 Space-scoped get/update/set-default/migration-preview；Default、默认 published Expert/ready Environment、manager RBAC、CAS、组织级幂等、审计/outbox 与真实 membership scope 由服务端强制 | 实际迁移 job、回滚、archive/delete 和 membership 管理尚未开放 |
| 权威配置与持久化 | PostgreSQL 已建立 Space authority、Expert/Environment identity、immutable revision、Automation Trigger/Event、provisioning lease/fencing、append-only audit/outbox、ExecutionSnapshot、Repository binding、ShareGrant、SessionWorker、Artifact、File/FileVersion、ToolCall/Approval/SideEffect 与复合 tenant FK；46 张租户表 FORCE RLS | File 内容仍为有界 `bytea`；大表 migration 的生产规模锁等待仍需演练 |
...
| 测试 | API/repository/config/JWT 单元测试；配置 `TEST_DATABASE_URL` 时运行 PostgreSQL 并发幂等、Space/Catalog/Automation/Session/SessionWorker/File/ToolCall/Approval 分页与可见性、Default invariant 与迁移预览、Event 去重/脱敏/ServiceAccount dispatch、不可变账本/配额、metadata/执行控制/ShareGrant、Worker hierarchy/version/state、provider→ToolCall→File→provider 续接、审批并发 CAS/职责分离/精确输入绑定、未知副作用阻断、受限角色/FORCE RLS/上下文防泄漏、跨 tenant/Private 隔离、Session FIFO、Worker 就绪边界和 `001 -> 069` 新库/升级测试；Session activity indexes 使用独立 concurrent migrations | 数据库测试会在无环境变量时 skip；无生产规模在线 schema、备份恢复或负载测试 |
...
| Create/start/send transaction | `start=true` create 同事务解析并锁定 Published ExpertRevision、Ready EnvironmentRevision 与 Repository binding，写入 immutable ExecutionSnapshot，再写 Session + first Message + Turn + Command + Outbox + 完整幂等响应；worker lease/heartbeat/fencing、单 Session FIFO 与只读 File Tool runtime 已实现 | 外部副作用虽有 ledger，但写工具尚未接入 provider，审批等待也未与 Command 租约形成可恢复编排 |
...
### ContextEngine-plugin 集成

ContextEngine-plugin 作为私网依赖运行，API 通过 Bearer credential 代理 `/status`、`/search` 与 `/context`；Web 不得持有插件 API key。Context 路由必须先验证 actor 的 Organization/Space membership，再验证 active Environment revision 的 repository binding，未绑定或未配置的 repository 统一返回 404，避免跨 Space 枚举。上游 `channels` 数值对象、`fileCount`、`chunkCount`、`hasEmbeddings` 与 `lastIndexedAt` 由 gateway 映射为共享契约，响应受大小、超时和 schema 校验约束。
