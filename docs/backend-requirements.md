...
| 权威配置与持久化 | PostgreSQL migration 已建立 Expert/Environment identity、immutable revision、Repository binding、ShareGrant、SessionWorker、Artifact、三作用域 File/FileVersion、ToolCall/Approval/SideEffect、ServiceAccount binding、复合 tenant FK，以及 33 张租户表和 1 张安全审计表的 FORCE RLS；SessionWorker/Session/Artifact/FileVersion/ToolCall 写边界维护版本或不可变 ledger；审批决定、副作用与 HMAC 脱敏的失败/拒绝账本不可变 | File 内容仍为有界 `bytea`；无 Expert/Environment CRUD/publish API 或 ExecutionSnapshot；大表 migration 的生产规模锁等待仍需演练 |
