...
| **Inferred** | 官方文档证明了组和入口，但没有完整证明 Configuration 的所有项目顺序、折叠方式以及品牌点击行为。 | 上述页面 | 当前 Configuration 同时包含 Experts、Environments、Daemons、Integrations、MCP、Webhooks、Secrets、Repositories、Spaces、Settings。 | 层级过长，且不能声称与 Cosmos 原版完全一致。 | 保留可折叠 Configuration 作为原型信息架构，但将顺序标为待视觉证据验证；品牌点击 Home 属于当前合理推断。 |
...
| 等级 | 官方事实或设计判断 | 来源 URL | 当前本地实现 | 差距 | 产品决策 |
| --- | --- | --- | --- | --- | --- |
| **Official** | Session 是与 Expert 的持续对话，包含消息、Agent turn 和 tool call。底部 composer 支持 `/` 命令、附件、拖放/粘贴图片、Enhance Prompt 和 Agent 工作期间的消息排队。会话头部支持 Share。详情面板展示 Artifacts；Worker tree 可从父 Session 查看。Cloud Session 的 Files tab 包含 Workspace、User、Organization 三个 scope。 | [Using Sessions](https://docs.augmentcode.com/cosmos/sessions-overview)；[Understanding Artifacts](https://docs.augmentcode.com/cosmos/artifacts)；[Delegating Work](https://docs.augmentcode.com/cosmos/workers-subagents)；[Understanding Files](https://docs.augmentcode.com/cosmos/understanding-files) | `RunWorkbench` 有 Agent、Terminal、Files、Subscriptions、Approval Tab；Agent 页有事件列表、附件名称、消息队列和 composer；右侧 Inspector 有只读 Artifact；Files 当前是代码 Diff；顶部有暂停、停止、重试。 | 缺少 Slash Command、Enhance Prompt、真实附件限制/预览、Share/成员授权、Artifact 增删改、Worker tree、三 scope Files；当前 Files 名称与官方 VFS Files 含义冲突。 | 以 Conversation 为主视图；补齐 composer、Share、Artifact 和 Worker tree。将现有 Diff 页改名为 Changes；新增官方语义的 Files tree。Subscriptions 放入右侧详情或 Automation 上下文。 |
