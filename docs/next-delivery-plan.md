...
> 更新日期：2026-07-21
>
> 基线提交：`66fa38a feat: complete expert lifecycle management`

## 结论

下一条垂直切片是 **Environment 生命周期与配置写入**。原因不是页面数量不足，而是它位于当前产品最关键的对象链路中：

```text
Environment revision → Expert published revision → Session execution snapshot
```

Expert 已完成创建、编辑、发布、禁用、归档、revision 历史、并发控制和运行时锁定。Environment 目前仍只有 tenant-scoped 的列表/详情读取，生产 Web 没有真实写入；而 `docs/api-contract.yaml` 已经定义了 Environment create/update/delete 的目标形态。没有 Environment 的真实生命周期，Expert 发布后的 Session 仍只能依赖预置数据，控制面闭环还没有完成。
...
| 领域 | 当前状态 | 影响 |
| --- | --- | --- |
| Environment | 仅列表和详情读取；没有真实 create/update/delete/provision/retry API | 管理员无法创建或更新可执行环境 |
