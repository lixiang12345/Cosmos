...
| 创建事务 | `start=true` 原子写 Session、首条 Message、首个 Turn、`session.start` Command、Outbox 和完整幂等响应；`start=false` 原子写 draft Session 与首条 Message，不创建 Turn、Command 或执行 Outbox | 所有命令统一使用领域事务、SessionEvent、Command、Outbox、幂等响应和必要审计 |
...
| ServiceAccount | token 可被认证；Session 路由目前没有 operation scope 分支，实际访问仍取决于 membership；Catalog 显式拒绝 ServiceAccount | 绑定 Organization、Space、Automation/Session、audience 与 operation scope；无通配默认权限 |
...
| 数据隔离 | 已有部分复合 tenant FK，repository SQL 在单条查询中重检 membership | 所有 Space 表全链路复合 tenant key + 强制 RLS/统一 tenant guard；运行角色不可绕过 |
| 生命周期 | 数据库可保存 Session/Turn/Command 状态，但只有 create 会产生状态；version 没有通用更新路径 | 本文全部合法转换、非法转换、归档正交语义和恢复语义 |
| 实时与审计 | 只有 Outbox 表；没有 SessionEvent、SSE、AuditEvent consumer | 持久化有序 SessionEvent、可恢复 SSE、append-only 脱敏审计 |
| 保留/删除 | 没有 Session retention job；普通 API 没有删除 Session | legal hold、幂等 retention job、deletion ledger、备份恢复后重放删除 |

当前准确名称仍是“权威 Session 配置与只读 Catalog 纵向切片”。上表 Target 未全部通过前不得标记生产可用或 GA。
...
| --- | --- | --- | --- |
| `draft` | `start` 或首个 `send` | `queued` | Message、Turn、Command、SessionEvent、Outbox；首次固定执行快照 |
...
当前数据库迁移 `001..007` 不能原地一次性变为完整 Target。生产迁移必须遵循 expand -> backfill -> validate -> switch -> contract：
