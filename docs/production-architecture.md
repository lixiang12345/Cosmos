...
| Operations | PostgreSQL 17 CI service；required workflow 运行 unit/build、非 skip integration、OpenAPI、dependency/Secret/whitespace/container scan、production migration 与 API/Web image/runtime/proxy smoke；每次 CI 生成三类容器 CycloneDX SBOM，并执行一次带 checksum 和目标库确认的逻辑备份/隔离恢复；本地 compose 只绑定 loopback；API/Worker/Web 有非 root 多阶段镜像与健康检查；API read-only readiness 与 Worker execution readiness 分离；API SQL 超时有界，Web 同源 `/api` upstream 与 CSP origin 通过运行时变量注入 | 无环境 IaC、镜像签名、生产 PITR/远端备份证据、staging/canary/rollback、SLO、on-call 和完整故障演练 |
