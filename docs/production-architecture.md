...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session、Approvals、Expert/Environment Catalog 与 User/Organization Files 调用 API；Files 支持服务端搜索/分页、文本预览、鉴权下载和不可变版本但无直接写入口；Approvals 提供权限过滤、证据、CAS 决策与双人进度；生产工作台轮询 canonical Message/Event 并投影真实执行状态；scope、凭据轮换或 401/403/404 会清除旧私密投影；demo 单独保留完整原型 | 基础对话、Approvals 与全局 Files 可作为服务端事实；Session 内 Tool/Workspace Files、Terminal 和 Changes 仍是未服务化范围 |
