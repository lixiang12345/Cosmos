...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session list/create/detail 与 Expert/Environment 只读 Catalog 调用 API；能力发现控制 `start=true`；生产工作台轮询 canonical Message/Event 并投影 running/retry/failure/completion；scope、凭据轮换或 401/403/404 会清除旧私密投影；demo 单独保留完整原型 | 基础对话状态可作为服务端事实；Tool、Files、Terminal、Changes 和 Approval 仍是未服务化范围 |
...
| API | Fastify；health/ready；`/me` identity discovery；capabilities；Session list/create/get；Message/Event list 与可恢复 SSE；Expert/Environment list/get；OIDC access token；Organization/Space membership 与角色交集；ServiceAccount 在 scope/binding 实现前 fail-closed | 无 Catalog 写 API、完整 Session 命令、Private share、细粒度 operation policy、限流和完整审计，不可公网暴露 |
| Data | PostgreSQL Expert/Environment immutable revision、Repository binding、Session/Turn/Command/Attempt/Message/Event；复合 tenant FK；actor/路径级幂等；Private creator 过滤；create 与 execution 状态转换写连续 SessionEvent；账本禁止 UPDATE/DELETE | 无 FORCE RLS/受限 runtime role、ExecutionSnapshot、ShareGrant 和完整领域实体；拒绝/失败审计与 retention 尚未实现 |
