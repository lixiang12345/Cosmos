...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session list/create/canonical detail 调用 API；列表和详情校验 tenant scope，凭据轮换隔离旧详情；其他主要页面仍为 mock/localStorage | 可演示，除已接 API 的 Session slice 外不可作为服务端权威数据 |
| Contract | `@relay/contracts` 对 Session create/list/get、authoritative revision IDs 和 v0/v1 兼容规则提供 Zod 运行时验证 | 与完整目标 OpenAPI 仍有已记录漂移，尚非单一契约源 |
| API | Fastify；health/ready；`/me` identity discovery；Session list/create/get；服务端解析 Published ExpertRevision、Ready EnvironmentRevision 与 Repository binding；OIDC access token；Organization/Space membership 与角色交集 | 无 Expert/Environment 管理 API、Private share、细粒度 operation policy、限流和领域审计，不可公网暴露 |
| Data | PostgreSQL Expert/Environment identity + immutable revision + Repository binding + Session；复合 tenant FK、current pointer 与不可变触发器；actor/规范路径级完整响应 idempotency；Private creator 过滤；启动时原子写首条 Message、Turn、Command 与 Outbox | 无 RLS/统一 tenant guard、ExecutionSnapshot 和完整领域实体；大表 migration 尚未按 expand/backfill/validate 分阶段；未满足 GA tenant isolation 深度防御 |
...
现阶段的准确名称是“权威 Session 配置纵向切片”，不是 MVP 后端完成或生产就绪。
