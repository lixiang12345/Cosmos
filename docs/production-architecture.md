...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session 列表与创建调用 API；其他主要页面仍为 mock/localStorage | 可演示，不可作为服务端权威数据 |
| Contract | `@relay/contracts` 为 Session 纵向切片提供 Zod 运行时验证 | 与 OpenAPI 存在已记录漂移，尚非单一契约源 |
| API | Fastify；health/ready；`/me` identity discovery；Session list/create；OIDC access token；Organization/Space membership 与角色交集；请求上限与 CORS 配置 | 无 Private share、细粒度 operation policy、限流和领域审计，不可公网暴露 |
| Data | PostgreSQL Session + actor/规范路径级完整响应 idempotency；Organization/Space FK 与 membership；Private creator 过滤；启动时原子写首条 Message、Turn、Command 与 Outbox；开发模式可用内存 repository | 无 RLS/统一 tenant guard 和完整领域实体；尚未满足 GA tenant isolation 深度防御 |
...
现阶段的准确名称是“Session 持久化纵向切片”，不是 MVP 后端完成或生产就绪。
