...
| **Current** | 2026-07-12 当前仓库中可指向代码/测试的实现 |
...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session list/draft create/canonical metadata detail 与 Expert/Environment 只读 Catalog 调用 API；scope 或凭据轮换立即隔离旧数据；按 actor role 隐藏创建入口；production capability allowlist 隐藏未服务化导航、命令和本地写操作，demo 单独保留完整原型 | 可作为服务端权威 metadata 的受限壳层；无 Message/Attempt/SSE 的执行结果不可作为生产事实 |
| Contract | `@relay/contracts` 对 Session create/list/get、authoritative revision IDs、Expert/Environment summary/detail 和分页响应提供 Zod 运行时验证 | 与完整目标 OpenAPI 仍有已记录漂移，尚非单一契约源 |
| API | Fastify；health/ready；`/me` identity discovery；Session list/create/get；Expert/Environment list/get；服务端解析 Published ExpertRevision、Ready EnvironmentRevision 与 Repository binding；OIDC access token；Organization/Space membership 与角色交集；ServiceAccount 在 scope/binding 实现前 fail-closed | 无 Catalog 写 API、Session lifecycle、Private share、细粒度 operation policy、限流和完整审计，不可公网暴露 |
| Data | PostgreSQL Expert/Environment identity + immutable revision + Repository binding + resource version、Session；Session child/idempotency 全链路复合 tenant FK；actor/规范路径级完整响应 idempotency；Private creator 过滤；create 原子写 Message、可选 Turn/Command/Outbox、连续 SessionEvent 与成功 AuditEvent；账本禁止 UPDATE/DELETE | 无 FORCE RLS/受限 runtime role、ExecutionSnapshot、Attempt 和完整领域实体；拒绝/失败审计与 retention 尚未实现；未满足 GA tenant isolation 深度防御 |
| Execution | 有 PostgreSQL Command/Outbox 接受队列 schema，但无 consumer、lease/heartbeat、真实 Agent runtime、sandbox、Tool Broker 和 SSE | 命令仅被持久接受；工作台执行结果仍是 Simulation |
| Operations | PostgreSQL 17 CI service；required workflow 运行 unit/build、非 skip integration、OpenAPI、Secret/whitespace scan、production migration 与 API/Web image/runtime/proxy smoke；本地 compose 只绑定 loopback；API/Web 有非 root 多阶段镜像与健康检查；API SQL 超时有界，Web 同源 `/api` upstream 与 CSP origin 通过运行时变量注入 | 无环境 IaC、镜像签名/SBOM、backup/restore、staging/canary/rollback、SLO、on-call 和故障演练 |

现阶段的准确名称是“权威 Session metadata、create success ledger 与只读 Catalog 纵向切片”，不是 MVP 后端完成或生产就绪。
