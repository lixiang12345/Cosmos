...
| Contract | `@relay/contracts` 对固定五模型目录、capabilities、Session、Message/Event timeline、Attempt/SessionWorker/Artifact/File/ToolCall/Approval 事件、Worker/ToolCall/Approval DTO、authoritative revision IDs、Expert/Environment summary/detail 和分页响应提供 Zod 运行时验证 | OpenAPI 与运行契约已覆盖当前表面，但尚未由同一源生成 |
| API | Fastify；health/ready；token-protected Prometheus HTTP/SSE metrics；`/me` identity discovery；capabilities；完整 Session metadata/execution controls；ShareGrant；Artifact/File/SessionWorker/ToolCall/Approval；Message/Event 与可恢复 SSE；每轮/每批 timeline query 重检 membership、ShareGrant、Group membership，撤权后关闭现有流；Expert/Environment list/get；ContextEngine-plugin Relay gateway（status/search/context）；OIDC；Organization/Space membership；ServiceAccount exact create/send/archive policy；严格代理信任、请求大小/超时、每实例 IP burst + PostgreSQL Organization shared quota limiter 和安全响应头；共享错误响应写独立、HMAC 脱敏的拒绝/失败账本 | 无 Catalog 写 API 和合规访问；公网仍需 Edge/WAF/全局限流 |
