...
| API | Fastify；health/ready；`/me` identity discovery；Session list/create/get；Expert/Environment list/get；服务端解析 Published ExpertRevision、Ready EnvironmentRevision 与 Repository binding；OIDC access token；Organization/Space membership 与角色交集 | 无 Catalog 写 API、Session lifecycle、Private share、细粒度 operation policy、限流和领域审计，不可公网暴露 |
| Data | PostgreSQL Expert/Environment identity + immutable revision + Repository binding + resource version、Session；复合 tenant FK、current pointer 与不可变触发器；actor/规范路径级完整响应 idempotency；Private creator 过滤；启动时原子写首条 Message、Turn、Command 与 Outbox | 无 RLS/统一 tenant guard、ExecutionSnapshot 和完整领域实体；大表 migration 尚未按 expand/backfill/validate 分阶段；未满足 GA tenant isolation 深度防御 |
...
现阶段的准确名称是“权威 Session 配置与只读 Catalog 纵向切片”，不是 MVP 后端完成或生产就绪。
