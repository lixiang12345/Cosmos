...
| API | Fastify；health/ready；`/me` identity discovery；capabilities；完整 Session metadata/execution controls；ShareGrant；Artifact/File/SessionWorker/ToolCall/Approval；Message/Event 与可恢复 SSE；Expert/Environment list/get；OIDC；Organization/Space membership；ServiceAccount exact create/send/archive policy；严格代理信任、请求大小/超时、每实例有界限流和安全响应头 | 无 Catalog 写 API、合规访问和完整拒绝/失败审计；公网仍需 Edge/WAF/全局限流 |
| Data | PostgreSQL immutable revision、Session/Turn/Command/Attempt/SessionWorker/Message/Event/ShareGrant/Artifact/File/FileVersion/ToolCall/Approval/SideEffect/ServiceAccount binding；复合 tenant FK；actor/路径级幂等；33 张租户表 FORCE RLS；API/Worker 分离的受限角色与 transaction-local tenant context；SessionWorker 状态、FileVersion、ApprovalDecision 和副作用账本禁止任意改写 | File 内容仍为有界 `bytea` 而非对象存储；无 ExecutionSnapshot；retention、生产备份恢复与规模迁移证据尚未实现 |
