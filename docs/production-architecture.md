...
| API | Fastify；health/ready；`/me` identity discovery；capabilities；Session keyset list/search/filter/create/get；Message/Event list 与可恢复 SSE；Expert/Environment list/get；OIDC access token；Organization/Space membership 与角色交集；ServiceAccount 在 scope/binding 实现前 fail-closed | 无 Catalog 写 API、完整 Session 命令、Private share、细粒度 operation policy、限流和完整审计，不可公网暴露 |
