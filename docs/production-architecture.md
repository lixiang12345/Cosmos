...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session list/create/canonical detail 与 Expert/Environment 只读 Catalog 调用 API；scope 或凭据轮换立即隔离旧数据；其他主要页面和 Session Workbench 仍为 mock/local state | 可演示；只有 Session metadata/create 和 Catalog query 是服务端权威，执行结果与其他控制面不可作为生产事实 |
...
| Operations | 有本地 compose PostgreSQL 和可选集成测试 | 无生产 CI/CD、backup/restore、SLO、on-call 和故障演练 |
