...
| Execution | PostgreSQL protocol-1 consumer 使用并发 claim、数据库权威 lease/heartbeat/fencing、有限重试、过期恢复和撤权取消；Worker 另写进程就绪心跳，并可通过内部 repository 创建/推进 Session-local 父子 Worker；OpenAI-compatible provider 固定五模型并按模型族选择凭据；Worker 内部 File writer 与 ToolCall coordinator 可原子写版本、精确 input hash Approval、双人决定、外部副作用状态和脱敏 ledger | provider 尚未触发 Session Worker 委派或 ToolCall coordinator；无 coding sandbox、Tool Broker、dead-letter 运维闭环与负载/soak 证据 |
