...
| Data | PostgreSQL Session + actor/Space 级 idempotency；Organization/Space FK 与 membership；Private creator 过滤；开发模式可用内存 repository | 无 RLS/统一 tenant guard 和完整领域实体；尚未满足 GA tenant isolation 深度防御 |
| Execution | 无真实 Agent runtime、queue、sandbox、Tool Broker 和 SSE | 工作台结果是 Simulation |
