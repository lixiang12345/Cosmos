...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session、Approvals、Expert/Environment Catalog 与 User/Organization/Session Workspace Files 调用 API；Files 支持服务端搜索/分页、文本预览、鉴权下载和不可变版本但无直接写入口，Workspace 查询精确绑定 Session；Approvals 提供权限过滤、证据、CAS 决策与双人进度；生产工作台轮询 canonical Message/Event 并投影真实执行状态；scope、凭据轮换或 401/403/404 会清除旧私密投影；demo 单独保留完整原型 | 基础对话、Approvals 与三类只读 Files 可作为服务端事实；Session 内独立 Tool、Terminal 和 Changes 仍是未服务化范围 |
| Contract | `@relay/contracts` 对固定五模型目录、capabilities、Session、Message/Event timeline、Attempt/Artifact/File/ToolCall/Approval 事件、ToolCall/Approval DTO、authoritative revision IDs、Expert/Environment summary/detail 和分页响应提供 Zod 运行时验证 | OpenAPI 与运行契约已覆盖当前表面，但尚未由同一源生成 |
| API | Fastify；health/ready；`/me` identity discovery；capabilities；完整 Session metadata/execution controls；ShareGrant；Artifact/File/ToolCall/Approval；Message/Event 与可恢复 SSE；Expert/Environment list/get；OIDC；Organization/Space membership；ServiceAccount exact create/send/archive policy | 无 Catalog 写 API、合规访问、限流和完整拒绝/失败审计，不可公网暴露 |
| Data | PostgreSQL immutable revision、Session/Turn/Command/Attempt/Message/Event/ShareGrant/Artifact/File/FileVersion/ToolCall/Approval/SideEffect/ServiceAccount binding；复合 tenant FK；actor/路径级幂等；32 张租户表 FORCE RLS；API/Worker 分离的受限角色与 transaction-local tenant context；FileVersion、ApprovalDecision 和副作用账本禁止任意改写 | File 内容仍为有界 `bytea` 而非对象存储；无 ExecutionSnapshot；retention、生产备份恢复与规模迁移证据尚未实现 |
| Execution | PostgreSQL protocol-1 consumer 使用并发 claim、数据库权威 lease/heartbeat/fencing、有限重试、过期恢复和撤权取消；Worker 另写进程就绪心跳；OpenAI-compatible provider 固定五模型并按 GPT/Claude/Grok 模型族选择凭据；Worker 内部 File writer 与 ToolCall coordinator 可原子写版本、精确 input hash Approval、双人决定、外部副作用状态和脱敏 ledger | ToolCall coordinator 尚未接入当前对话 provider；无 coding sandbox、Tool Broker、dead-letter 运维闭环与负载/soak 证据 |
