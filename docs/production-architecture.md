...
| API | Fastify；health/ready；Session list/create；请求上限与 CORS 配置 | 无身份、授权、限流和领域审计，不可公网暴露 |
| Data | PostgreSQL Session + idempotency migration；开发模式可用内存 repository | 无 FK/RLS/完整领域实体，不满足 tenant isolation |
