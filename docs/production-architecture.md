...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session、Expert/Environment Catalog 与 User/Organization Files 调用 API；Files 支持服务端搜索/分页、文本预览、鉴权下载和不可变版本但无直接写入口；生产工作台轮询 canonical Message/Event 并投影真实执行状态；scope、凭据轮换或 401/403/404 会清除旧私密投影；demo 单独保留完整原型 | 基础对话与全局 Files 可作为服务端事实；Tool、Workspace Files、Terminal、Changes 和 Approval 仍是未服务化范围 |
...
| Data | PostgreSQL immutable revision、Session/Turn/Command/Attempt/Message/Event/ShareGrant/Artifact/File/FileVersion/ServiceAccount binding；复合 tenant FK；actor/路径级幂等；27 张租户表 FORCE RLS；API/Worker 分离的受限角色与 transaction-local tenant context；FileVersion 和账本禁止 UPDATE/DELETE | File 内容仍为有界 `bytea` 而非对象存储；无 ExecutionSnapshot；retention、生产备份恢复与规模迁移证据尚未实现 |
| Execution | PostgreSQL protocol-1 consumer 使用并发 claim、数据库权威 lease/heartbeat/fencing、有限重试、过期恢复和撤权取消；Worker 另写进程就绪心跳，`capabilities` 与新 `start=true` 只在心跳新鲜时开放；OpenAI-compatible provider 只生成有界对话输出；Worker 内部 File append repository 可原子写版本和脱敏 ledger | 无 coding sandbox、Tool Broker/ToolCall 编排、外部副作用 ledger、dead-letter 运维闭环与负载/soak 证据 |
