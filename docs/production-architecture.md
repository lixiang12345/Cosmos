...
| Data | PostgreSQL immutable revision、Session/Turn/Command/Attempt/Message/Event/ShareGrant/ServiceAccount binding；复合 tenant FK；actor/路径级幂等；24 张租户表 FORCE RLS；API/Worker 分离的受限角色与 transaction-local tenant context；账本禁止 UPDATE/DELETE | 无 ExecutionSnapshot 和完整领域实体；retention、生产备份恢复与规模迁移证据尚未实现 |
