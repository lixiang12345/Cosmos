...
本文定义 Cosmos 处理真实客户数据时的系统边界、信任边界、容灾目标和发布门槛。它不用于证明架构已经落地；实施状态必须由代码、部署和可复现验证证明。
...
| **Cosmos target** | Cosmos 为安全、可靠性、治理和商业交付定义的架构约束 |
...
| 层 | **Current** | 生产判定 |
| --- | --- | --- |
| Web | React/Vite；Session、Approvals、Expert/Environment Catalog、User/Organization/Session Workspace Files 与 Session Worker tree 调用 API；Files 支持服务端搜索/分页、文本预览、鉴权下载和不可变版本但无直接写入口；Worker tree 按父节点优先显示状态、指令与结果；生产工作台轮询 canonical Message/Event 并投影真实执行状态；scope、凭据轮换或 401/403/404 会清除旧私密投影；demo 单独保留完整原型 | 基础对话、Approvals、三类只读 Files 与 Worker tree 可作为服务端事实；Session 内独立 Tool、Terminal 和 Changes 仍是未服务化范围 |
| Contract | `@cosmos/contracts` 对固定五模型目录、capabilities、Session、Message/Event timeline、Attempt/SessionWorker/Artifact/File/ToolCall/Approval 事件、Worker/ToolCall/Approval DTO、authoritative revision IDs、Expert/Environment summary/detail 和分页响应提供 Zod 运行时验证 | OpenAPI 与运行契约已覆盖当前表面，但尚未由同一源生成 |
| API | Fastify；health/ready；token-protected Prometheus HTTP/SSE metrics；`/me` identity discovery；capabilities；完整 Session metadata/execution controls；ShareGrant；Artifact/File/SessionWorker/ToolCall/Approval；Message/Event 与可恢复 SSE；每轮/每批 timeline query 重检 membership、ShareGrant、Group membership，撤权后关闭现有流；Expert/Environment list/get；ContextEngine-plugin Cosmos gateway（status/search/context）；OIDC；Organization/Space membership；ServiceAccount exact create/send/archive policy；严格代理信任、请求大小/超时、每实例 IP burst + PostgreSQL Organization shared quota limiter 和安全响应头；共享错误响应写独立、HMAC 脱敏的拒绝/失败账本 | 无 Catalog 写 API 和合规访问；公网仍需 Edge/WAF/全局限流 |
| Data | PostgreSQL immutable revision、Session/Turn/Command/Attempt/SessionWorker/Message/Event/ShareGrant/Artifact/File/FileVersion/ToolCall/Approval/SideEffect/ServiceAccount binding；Organization quota 与共享 rate-limit window；复合 tenant FK；actor/路径级幂等；tenant 表 FORCE RLS；API/Worker 分离的受限角色与 transaction-local tenant context；SessionWorker 状态、FileVersion metadata、ApprovalDecision、副作用和 SecurityAudit 账本禁止任意改写；FileVersion 支持带 checksum 的 S3-compatible object backend 并保留历史 inline 兼容 | 生产 bucket/IAM、加密/保留策略、对象恢复部署证据尚未完成 |
| Execution | PostgreSQL protocol-1 consumer 使用并发 claim、数据库权威 lease/heartbeat/fencing、有限重试、过期恢复和撤权取消；Automation Session 固定 autoArchive snapshot，并在最终 Turn 成功事务中写自动归档 Event/Audit；Worker 另写进程就绪心跳，并可通过内部 repository 创建/推进 Session-local 父子 Worker；OpenAI-compatible provider 固定五模型并按模型族选择凭据；provider 可在同一租约内最多 8 轮调用受控 Workspace File list/read Tool Broker；完整服务端配置时另开放固定 HTTPS 目的地的 `approved_webhook_delivery`，独立人类 Approval 绑定精确 input hash 后才准备幂等 SideEffect，2xx/4xx/unknown 严格分流；等待审批期间维持 lease，过期 Approval 由 Worker 恢复循环安全取消并恢复父执行 | provider 尚未触发 Session Worker 委派，也未开放任意 URL、Shell 或代码写入；无 coding sandbox、通用外部 provider 工具、dead-letter 运维闭环与目标环境 soak 证据 |
| Operations | PostgreSQL 17 CI service；required workflow 运行 unit/build、非 skip integration、OpenAPI、dependency/Secret/whitespace/container scan、production migration 与 API/Web image/runtime/proxy smoke；每次 CI 生成三类容器 CycloneDX SBOM，执行带 checksum、release/RLS/ACL/数据约束验证的逻辑备份/隔离恢复，并在独立 PostgreSQL 上强制 WAL switch 验证连续归档 preflight；仓库包含 Prometheus SLO/alert rules、独立 observer 数据库指标链路、Worker heartbeat readiness gauge、page/ticket Runbook、有界 HTTP smoke、Session 生命周期 journey，以及可恢复的 Worker/PostgreSQL readiness drill；core Outbox 另有 metadata-only HTTPS dispatcher、`SKIP LOCKED` lease/fencing、有界 retry/dead-letter、versioned replay CLI 和 append-only operational audit；本地 compose 只绑定 loopback；API/Worker/Web 有非 root 多阶段镜像与健康检查；API read-only readiness 与 Worker execution readiness 分离；API SQL 超时有界，Web 同源 `/api` upstream 与 CSP origin 通过运行时变量注入 | 无环境 IaC、镜像签名、生产托管 PITR/跨账号远端备份证据、实际 receiver/on-call ACK、catalog-specific Outbox consumer、staging/canary/rollback、目标环境 execution soak 和完整故障演练 |
...
该图全部是 **Cosmos target**。各实施可同进程起步，但不可合并信任责任：API 不能绕过 Tool Policy 执行外部写，Agent runtime 不能直接给自己提权，投影/搜索不能取代 PostgreSQL 权威数据。
...
5. API 返回 201；命令已接受不等于 Agent 已执行。Outbox cosmos 将 Command 交付给队列。
...
4. 外部写使用 provider idempotency key 或 Cosmos side-effect ledger；租约丢失时不盲目重放未知结果。
...
## 7. 可靠性和 SLO（Cosmos 初始目标）

下表是 **Cosmos target**，不是当前测量结果。设计合作伙伴阶段可收集基线并调整，但降低目标需要产品、工程和安全共同记录决策。
