...
| Operations | PostgreSQL 17 CI service；required workflow 运行 unit/build、非 skip integration、OpenAPI、dependency/Secret/whitespace/container scan、production migration 与 API/Web image/runtime/proxy smoke；每次 CI 生成三类容器 CycloneDX SBOM，执行带 checksum、release/RLS/ACL/数据约束验证的逻辑备份/隔离恢复，并在独立 PostgreSQL 上强制 WAL switch 验证连续归档 preflight；仓库包含 Prometheus SLO/alert rules、独立 observer 数据库指标链路、Worker heartbeat readiness gauge、page/ticket Runbook、有界 HTTP smoke、Session 生命周期 journey，以及可恢复的 Worker/PostgreSQL readiness drill；本地 compose 只绑定 loopback；API/Worker/Web 有非 root 多阶段镜像与健康检查；API read-only readiness 与 Worker execution readiness 分离；API SQL 超时有界，Web 同源 `/api` upstream 与 CSP origin 通过运行时变量注入 | 无环境 IaC、镜像签名、生产托管 PITR/跨账号远端备份证据、实际 dashboard/receiver/on-call、staging/canary/rollback、目标环境 execution soak 和完整故障演练 |
