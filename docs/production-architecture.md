...
| API | Fastify；health/ready；Session list/create；OIDC access token；Organization/Space membership；viewer 写限制；请求上限与 CORS 配置 | 无 Private share、细粒度 operation policy、限流和领域审计，不可公网暴露 |
| Data | PostgreSQL Session + actor/Space 级完整响应 idempotency；Organization/Space FK 与 membership；Private creator 过滤；启动时原子写首条 Message、Turn、Command 与 Outbox；开发模式可用内存 repository | 无 RLS/统一 tenant guard 和完整领域实体；尚未满足 GA tenant isolation 深度防御 |
