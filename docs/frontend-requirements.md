...
| 范围 | 当前事实 | 状态 | 生产缺口 |
| --- | --- | --- | --- |
| Session 列表、创建与详情 | `getMe` 发现 actor 的 Organization/Space membership；list/create/get/rename/archive/restore/start/send 使用当前合法 scope；生产列表增量读取服务端 cursor，并可加载归档记录；详情规范路由为 `/sessions/:sessionId`，直刷读取单资源 API，旧 `/runs/:id` 重定向；Web 已接 OIDC Code + PKCE、Bearer、401 失效和生产 fail-closed | **Partial** | 搜索/状态筛选尚未绑定服务端查询；Artifact、Worker、Pin 和执行生命周期命令未接 API；真实 IdP E2E 待配置 |
