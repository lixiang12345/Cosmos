...
配置 `DATABASE_URL` 后，Expert/Environment identity 与 immutable revision、Repository binding、Session、Attempt、ToolCall/Approval/SideEffect、Artifact 引用、三作用域 File/FileVersion、事件和幂等记录写入 PostgreSQL；未配置时仅开发环境使用进程内存 repository。API 已实现 OIDC、membership discovery、User/Group ShareGrant、ServiceAccount exact create/send/archive binding、完整 Session 生命周期命令、Artifact 管理、ToolCall 查询、Approval 查询/决策、只读 File 浏览/版本/内容下载、复合 tenant FK，以及由事务本地 actor/org/space context 驱动的 32 张租户表 FORCE RLS；API/Worker 使用分离的受限数据库角色。FileVersion 当前以 PostgreSQL `bytea` 保存，单版本限制 1 MiB、单 Organization 默认写入配额 100 MiB；Worker 内部 repository 原子写不可变文件版本、精确 input hash 审批、双人批准、外部副作用状态和脱敏 Event/Audit/Outbox。ToolCall coordinator 尚未接入当前对话 provider；Expert/Environment 写 API、拒绝与失败的完整审计、coding sandbox、Tool Broker、对象存储以及生产备份/恢复/负载证据仍未实现，因此当前版本仍不能直接暴露到公网。这些能力按 [软件交付计划](./docs/software-delivery-plan.md) 继续演进。
