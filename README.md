...
可选的核心 Outbox dispatcher 仅在 Worker 同时获得 `OUTBOX_RECEIVER_URL` 与 `OUTBOX_RECEIVER_BEARER_TOKEN` 时启动。它对 Session/Environment/Automation/Space 事件使用 PostgreSQL lease/fencing、稳定幂等键、有界指数退避和显式 dead-letter；外发 envelope 不含 source payload 或 receiver body。运维通过 metadata-only `pnpm outbox:dead-letter` list/replay，replay 受 status/version fence 和固定 reason code 约束。完整配置与处置边界见 [Core Outbox Delivery Runbook](./docs/outbox-delivery-runbook.md)。
...
- 当前 Agent provider 可产出受大小限制的对话 Message，或调用 `workspace_files_list` / `workspace_file_read` 两个严格参数、单次 64 KiB 上限的只读工具；Worker 在同一租约内以最多 8 轮的模型续接执行，并把每次调用写入 ToolCall/Event/Audit。provider endpoint 的 301/302/303/307/308 重定向不会被跟随，并按终止型配置错误处理。对 OpenAI-compatible 中转仅兼容两类已知偏差：工具回合缺省 `content`，以及工具参数前置单个空对象分片；响应仍受原始字节、JSON 复杂度、工具 allowlist 和输出字符边界约束，HTML 或其他畸形响应继续失败关闭。SessionEvent 以单调 sequence 持久化，并可通过 cursor 分页或 `Last-Event-ID` 恢复 SSE。SSE heartbeat 会重新认证；每轮/每批 timeline 查询都会重检当前 membership、ShareGrant 和 Group membership，撤权后在下一次检查关闭连接。
