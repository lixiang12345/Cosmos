      ...
      <div className="cosmos-page__scroll">
        <div className="cosmos-settings-layout">
          ...
          <section className="cosmos-panel cosmos-settings-panel">
            ...
            {section === 'appearance' ? <><header className="cosmos-section-heading"><div><span>{copy(locale, '界面', 'Interface')}</span><h2>{copy(locale, '外观与语言', 'Appearance & language')}</h2></div></header><div className="cosmos-preference-list"><div className="cosmos-preference-row"><span className="cosmos-preference-row__identity"><SunMoon aria-hidden="true" /><span><strong>{copy(locale, '主题', 'Theme')}</strong><small>{copy(locale, '立即应用并持久化到当前浏览器。', 'Applies immediately and persists in this browser.')}</small></span></span><div className="cosmos-preference-segmented" role="group" aria-label={copy(locale, '主题', 'Theme')}><button type="button" className={theme === 'light' ? 'cosmos-preference-segmented__button cosmos-preference-segmented__button--active' : 'cosmos-preference-segmented__button'} aria-pressed={theme === 'light'} onClick={() => setTheme('light')}><Sun aria-hidden="true" />{copy(locale, '浅色', 'Light')}</button><button type="button" className={theme === 'dark' ? 'cosmos-preference-segmented__button cosmos-preference-segmented__button--active' : 'cosmos-preference-segmented__button'} aria-pressed={theme === 'dark'} onClick={() => setTheme('dark')}><Moon aria-hidden="true" />{copy(locale, '深色', 'Dark')}</button></div></div><label className="cosmos-preference-row"><span className="cosmos-preference-row__identity"><Languages aria-hidden="true" /><span><strong>{copy(locale, '语言', 'Language')}</strong><small>{copy(locale, '切换整个原型的界面语言。', 'Switches the interface language across the prototype.')}</small></span></span><select value={locale} onChange={(event) => setLocale(event.target.value as Locale)}><option value="zh">简体中文</option><option value="en">English</option></select></label></div><footer className="cosmos-settings-panel__footer"><button type="button" className="cosmos-button cosmos-button--ghost" onClick={() => { setTheme('dark'); setLocale('zh'); setNotice('已恢复默认外观偏好') }}><RefreshCw aria-hidden="true" />{copy(locale, '恢复默认', 'Restore defaults')}</button></footer></> : null}
          </section>
        </div>
      </div>
    </main>
