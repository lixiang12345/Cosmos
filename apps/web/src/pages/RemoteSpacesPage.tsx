import type { MeSpace, SpaceDto, SpaceMigrationDto, SpaceMigrationPreview } from '@cosmos/contracts'
import {
  CheckCircle2,
  ChevronRight,
  CircleDot,
...
export function RemoteSpacesPage(props: Props) {
  ...
  const [migrations, setMigrations] = useState<SpaceMigrationDto[]>([])
  ...
  const runMigration = () => {
    ...
    setMigrating(true)
    setError(undefined)
    void executeSpaceMigration(props.organizationId, props.activeSpaceId, {
      targetSpaceId: preview.target.id,
      resourceType: 'webhooks',
    }, key('space-migration'), props.auth).then((migration) => {
      setMigrations((current) => [migration, ...current.filter((item) => item.id !== migration.id)])
      setNotice(migration.status === 'completed'
        ? copy(locale, `已迁移 ${migration.resourceMigrated} 个 Webhook 到 ${preview.target.name}。`, `Migrated ${migration.resourceMigrated} webhook(s) to ${preview.target.name}.`)
        : copy(locale, '迁移未完成，详见下方作业记录。', 'The migration did not complete; see the job record below.'))
      setPreview(undefined)
      refreshMigrations()
    }, (cause: unknown) => {
      setError(cause instanceof Error ? cause : new Error('Unable to execute the migration.'))
      refreshMigrations()
