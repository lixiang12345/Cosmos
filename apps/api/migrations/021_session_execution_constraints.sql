...
ALTER TABLE relay_attempts
  ADD CONSTRAINT relay_attempts_tenant_identity_unique
  UNIQUE USING INDEX relay_attempts_tenant_identity_unique;

ALTER TABLE relay_attempts
  ADD CONSTRAINT relay_attempts_turn_number_unique
  UNIQUE USING INDEX relay_attempts_turn_number_unique;

ALTER TABLE relay_attempts
  ADD CONSTRAINT relay_attempts_turn_tenant_fk
...
ALTER TABLE relay_messages
  ADD CONSTRAINT relay_messages_turn_tenant_fk
  FOREIGN KEY (organization_id, space_id, session_id, turn_id)
  REFERENCES relay_turns(organization_id, space_id, session_id, id)
  ON DELETE RESTRICT NOT VALID,
  ADD CONSTRAINT relay_messages_attempt_tenant_fk
...
ALTER TABLE relay_session_events
  ADD CONSTRAINT relay_session_events_attempt_tenant_fk
  FOREIGN KEY (organization_id, space_id, session_id, turn_id, attempt_id)
  REFERENCES relay_attempts(organization_id, space_id, session_id, turn_id, id)
  ON DELETE RESTRICT NOT VALID;

ALTER TABLE relay_commands
  ADD CONSTRAINT relay_commands_protocol_version_check
...
ALTER TABLE relay_attempts
  ADD CONSTRAINT relay_attempts_number_check
  CHECK (number > 0) NOT VALID,
  ADD CONSTRAINT relay_attempts_model_check
  CHECK (btrim(model) <> '') NOT VALID,
  ADD CONSTRAINT relay_attempts_runtime_tuple_check
  CHECK (
    (
      status = 'queued'
...
ALTER TABLE relay_messages
  ADD CONSTRAINT relay_messages_agent_attempt_check
  CHECK (
...
ALTER TABLE relay_session_events
  ...
  ADD CONSTRAINT relay_session_events_runtime_actor_kind_check
...
ALTER TABLE relay_session_events
  DROP CONSTRAINT relay_session_events_event_type_check,
  DROP CONSTRAINT relay_session_events_resource_type_check,
  DROP CONSTRAINT relay_session_events_actor_kind_check,
  DROP CONSTRAINT relay_session_events_typed_resource_check;

CREATE FUNCTION relay_protect_attempt_history()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP IN ('DELETE', 'TRUNCATE') THEN
    RAISE EXCEPTION 'Relay Attempt rows cannot be deleted'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF NEW.organization_id IS DISTINCT FROM OLD.organization_id
    ...
    RAISE EXCEPTION 'Relay Attempt identity is immutable'
      USING ERRCODE = '55000';
  END IF;

  IF OLD.status IN ('succeeded', 'failed', 'canceled') THEN
    RAISE EXCEPTION 'Terminal Relay Attempt rows are immutable'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF NEW.status <> OLD.status AND NOT (
    ...
    RAISE EXCEPTION 'Invalid Relay Attempt transition from % to %', OLD.status, NEW.status
      USING ERRCODE = '23514';
  END IF;

  IF OLD.started_at IS NOT NULL AND NEW.started_at IS DISTINCT FROM OLD.started_at THEN
    RAISE EXCEPTION 'Relay Attempt started_at is immutable once set'
      USING ERRCODE = '55000';
  END IF;

  IF OLD.runtime_id IS NOT NULL AND NEW.runtime_id IS DISTINCT FROM OLD.runtime_id THEN
    RAISE EXCEPTION 'Relay Attempt runtime_id is immutable once set'
      USING ERRCODE = '55000';
  END IF;

  IF OLD.heartbeat_at IS NOT NULL
     AND (NEW.heartbeat_at IS NULL OR NEW.heartbeat_at < OLD.heartbeat_at) THEN
    RAISE EXCEPTION 'Relay Attempt heartbeat_at cannot move backwards'
      USING ERRCODE = '23514';
  END IF;
...
CREATE TRIGGER relay_attempts_protect_history
  BEFORE UPDATE OR DELETE ON relay_attempts
  FOR EACH ROW EXECUTE FUNCTION relay_protect_attempt_history();

CREATE TRIGGER relay_attempts_reject_truncate
  BEFORE TRUNCATE ON relay_attempts
  FOR EACH STATEMENT EXECUTE FUNCTION relay_protect_attempt_history();

REVOKE DELETE, TRUNCATE ON relay_attempts FROM PUBLIC;

CREATE FUNCTION relay_protect_command_runtime()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF NEW.protocol_version IS DISTINCT FROM OLD.protocol_version THEN
    RAISE EXCEPTION 'Relay Command protocol_version is immutable'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF ROW(
    ...
    RAISE EXCEPTION 'Relay Command request identity is immutable'
      USING ERRCODE = '55000';
  END IF;

  IF OLD.status IN ('succeeded', 'failed', 'canceled') THEN
    RAISE EXCEPTION 'Terminal Relay Command rows are immutable'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF NEW.status <> OLD.status AND NOT (
    ...
    RAISE EXCEPTION 'Invalid Relay Command transition from % to %', OLD.status, NEW.status
      USING ERRCODE = '23514';
  END IF;

  IF NEW.attempts < OLD.attempts OR NEW.attempts > OLD.attempts + 1 THEN
    RAISE EXCEPTION 'Relay Command attempts fence must advance monotonically by one'
      USING ERRCODE = '23514';
  END IF;

  IF NEW.status = 'running' AND OLD.status IN ('accepted', 'queued')
     AND NEW.attempts <> OLD.attempts + 1 THEN
    RAISE EXCEPTION 'Claiming a Relay Command must advance its attempts fence'
      USING ERRCODE = '23514';
  END IF;
  ...
  IF OLD.status = 'running' AND NEW.status = 'running'
    ...
    RAISE EXCEPTION 'Relay Command lease ownership changes require a new attempts fence'
      USING ERRCODE = '23514';
  END IF;

  IF NEW.status <> 'running' AND NEW.attempts <> OLD.attempts THEN
    RAISE EXCEPTION 'Relay Command attempts may advance only while claiming a lease'
      USING ERRCODE = '23514';
  END IF;

  IF OLD.heartbeat_at IS NOT NULL AND NEW.status <> 'queued'
     AND (NEW.heartbeat_at IS NULL OR NEW.heartbeat_at < OLD.heartbeat_at) THEN
    RAISE EXCEPTION 'Relay Command heartbeat_at cannot move backwards'
      USING ERRCODE = '23514';
  END IF;
...
CREATE TRIGGER relay_commands_protect_runtime
  BEFORE UPDATE ON relay_commands
  FOR EACH ROW EXECUTE FUNCTION relay_protect_command_runtime();
